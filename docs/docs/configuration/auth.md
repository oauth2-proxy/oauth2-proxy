...
```
    ...
    --validate-url="https://< your gitea host >/api/v1/user"
```

The Gitea auth provider supports two additional ways to restrict authentication to either organization and optional team level access, or to collaborators of a repository. Restricting by these options is normally accompanied with `--email-domain=*`

NOTE: When `--gitea-user` is set, the specified users are allowed to login even if they do not belong to the specified org and team or collaborators.

To restrict by organization only, include the following flag:

    -gitea-org="": restrict logins to members of this organisation

To restrict within an organization to a specific team, include the following flag in addition to `-gitea-org`:

    -gitea-team="": restrict logins to members of any of this team

If you would rather restrict access to collaborators of a repository, those users must either have push access to a public repository or any access to a private repository:

    -gitea-repo="": restrict logins to collaborators of this repository formatted as orgname/repo

To allow a user to login with their username even if they do not belong to the specified org and team or collaborators, separated by a comma

    -gitea-user="": allow logins by username, separated by a comma
