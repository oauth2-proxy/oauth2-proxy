...
:::warning
This page contains documentation for beta features.
...
Please beware of this before you use beta configuration options.
:::

This page details a set of **beta** configuration options in a new format.
...
[YamlOptions](#yamloptions) as the root of the configuration.

When using yaml configuration, your config file will look something like below:
...
To use the new **beta** configuration, generate a YAML file based on the format
described in the [reference](#configuration-reference) below.

Provide the path to this file using the `--config` flag.

:::note
When using the new config format, some options are no longer available.
See [removed options](#removed-options) below for more information.
:::

### Converting configuration to the new structure

Before using the new configuration format, start OAuth2 Proxy using the
...
```bash
oauth2-proxy --config ./path/to/new/config.yaml --legacy-config ./path/to/existing/config.cfg
```
...
use any of these options via flags or via the legacy config is going to result in
an error.

:::important
You must remove these options before starting OAuth2 Proxy with the new config format.
