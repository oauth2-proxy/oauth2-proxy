...
Note that [nginxinc/kubernetes-ingress](https://github.com/nginxinc/kubernetes-ingress) does not include the lua module for this by default.
...
```
It is recommended to use an external redis for your session management via `--session-store-type=redis` when expecting large sessions/OIDC tokens (_e.g._ with MS Azure).
