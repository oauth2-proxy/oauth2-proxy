...
| flag: `--cookie-partitioned`<br/>toml: `cookie_partitioned`                         | bool           | set the [Partitioned cookie attribute](https://developer.mozilla.org/en-US/docs/Web/Privacy/Guides/Privacy_sandbox/Partitioned_cookies) (CHIPS); requires `--cookie-secure=true`. For cross-site iframe use, `--cookie-samesite=none` may also be needed. | false             |
