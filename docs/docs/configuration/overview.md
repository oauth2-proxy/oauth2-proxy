...
| flag: `--deny-invalid-bearer-tokens`<br/>toml: `deny_invalid_bearer_tokens`       | bool           | if `--skip-jwt-bearer-tokens` is set, if a request includes an invalid JWT (expired, malformed, missing required audiences, etc), respond with 403 instead of falling back to authorizatiion code grant redirect | false       |
...
| flag: `--skip-jwt-bearer-tokens`<br/>toml: `skip_jwt_bearer_tokens`       | bool           | will skip requests that have verified JWT bearer tokens (the token must have [`aud`](https://en.wikipedia.org/wiki/JSON_Web_Token#Standard_fields) that matches this client id or one of the extras from `extra-jwt-issuers`). If a JWT is not valid, redirects to authorization server for login by default, unless `--deny-invalid-bearer-tokens` is set | false       |
