...
| flag: `--skip-auth-preflight`<br/>toml: `skip_auth_preflight`                 | bool           | will skip authentication for OPTIONS requests. In forward-auth / API mode, `X-Forwarded-Method` is used to determine the original request method, subject to the same trust model as other `X-Forwarded-*` headers (see `--reverse-proxy` and `--trusted-proxy-ip`). Ensure the upstream proxy sets `X-Forwarded-Method` from the real request method rather than forwarding the client-supplied value.                                                                                                             | false       |
