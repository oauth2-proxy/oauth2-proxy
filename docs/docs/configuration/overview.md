...
| flag: `--deny-invalid-bearer-tokens`<br/>toml: `deny_invalid_bearer_tokens`       | bool           | if `--skip-jwt-bearer-tokens` is set, if a request includes an invalid JWT (expired, malformed, missing required audiences, etc), respond with 403 instead of falling back to authorization code grant redirect | false       |
