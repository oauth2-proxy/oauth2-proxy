...
| `--azure-federated-token-auth-enabled` | bool | Whether to authenticate to Azure using federated credential. Token path is read from `$AZURE_FEDERATED_TOKEN_FILE` - default for Azure workload identity webhook server | false |
