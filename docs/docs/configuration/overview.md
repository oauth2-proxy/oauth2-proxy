...
| flag: `--retry-csrf-errors`<br/>toml: `retry_csrf_errors`                         | bool           | If set to true the oauth_proxy will automatically restart the authentication flow if an error with CSRF cookies occurs. To work `max-automated-retries` also needs to be increased.                                                                   | false             |
