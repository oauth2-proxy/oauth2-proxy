...
| `--redis-password` | string | Redis password. Applicable for all Redis configurations. Will override any password set in `--redis-connection-url`. Can be specified as environment variable `REDIS_PASSWORD`. | |
| `--redis-sentinel-password` | string | Redis sentinel password. Used only for sentinel connection; any redis node passwords need to use `--redis-password`. Can be specified as environment variable `REDIS_SENTINEL_PASSWORD`.  | |
