...
| `--generated-token-type` | string | If not `"none"`, add an additional button to the sign_in page. Using this button will generate and display a token of this type, instead of logging into the application. Valid options are `"none"`, `"access"` and `"refresh"`. `"access"` tokens can be used with `--skip-jwt-bearer-tokens`, and `"refresh"` can be used with `--exchange-refresh-bearer-tokens` | `"none"` |
| `--generated-token-scope` | string | Scope will be used for tokens generated by the page enabled by `--generated-token-type` | Same as `scope` |
