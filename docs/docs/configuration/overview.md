...
| `--extra-jwt-issuers` | string | if `--skip-jwt-bearer-tokens` is set, a list of extra JWT `issuer=audience` (see a token's `iss`, `aud` fields) pairs (where the issuer URL has a `.well-known/openid-configuration` or a `.well-known/jwks.json`). If the IDP does not support the `.well-known` URL convention, a JWKS URI for the issuer can optionally be specified by adding it in between the issuer and audience fields, e.g. `issuer=jwksURI=audience` | |
