...
| flag: `--cookie-partitioned`<br/>toml: `cookie_partitioned`                         | bool           | set the [Partitioned cookie attribute](https://developer.mozilla.org/en-US/docs/Web/Privacy/Guides/Privacy_sandbox/Partitioned_cookies) (CHIPS), useful when oauth2-proxy is used in a cross-site iframe while third-party cookies are blocked. Requires `--cookie-secure=true` and `--cookie-samesite=none`; `cookie-csrf-samesite` must be unset or `none`. | false             |
