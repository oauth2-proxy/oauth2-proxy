...
| flag: `--skip-auth-strip-headers`<br/>toml: `skip_auth_strip_headers`     | bool   | strips `X-Forwarded-*` style authentication headers, `Authorization` header and CSRF token client header if they would be set by oauth2-proxy                                                                                                                                                | true    |
...
| Flag / Config Field                                                       | Type           | Description                                                                                                                                                                                                          | Default                     |
| ------------------------------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| flag: `--csrftoken`<br/>toml: `csrftoken`                                 | bool           | Enable CSRF token generation mechanism in oauth2-proxy.                                                                                                                                                              | false                       |
| flag: `--csrftoken-cookie-name`<br/>toml: `csrftoken_cookie_name`         | string         | The name of the CSRF cookie. If set to empty string, no CSRF cookie will be set by oauth2-proxy.                                                                                                                     | `"_oauth2_proxy_csrftoken"` |
| flag: `--csrftoken-cookie-domain`<br/>toml: `csrftoken_cookie_domain`     | string \| list | Optional cookie domains to set CSRF cookies to (e.g. `.yourcompany.com`). The longest domain matching the request's host will be used (or the shortest cookie domain if there is no match).                          |                             |
| flag: `--csrftoken-cookie-path`<br/>toml: `csrftoken_cookie_path`         | string         | An optional cookie path to set CSRF cookies to (e.g. `/poc/`).                                                                                                                                                       | `"/"`                       |
| flag: `--csrftoken-cookie-secure`<br/>toml: `csrftoken_cookie_secure`     | bool           | set [secure (HTTPS only) cookie flag](https://owasp.org/www-community/controls/SecureCookieAttribute).                                                                                                               | true                        |
| flag: `--csrftoken-cookie-httponly`<br/>toml: `csrftoken_cookie_httponly` | bool           | Enable HttpOnly for CSRF cookie.                                                                                                                                                                                     | false                       |
| flag: `--csrftoken-cookie-samesite`<br/>toml: `csrftoken_cookie_samesite` | string         | Set SameSite cookie attribute for CSRF cookie (`"lax"`, `"strict"`, `"none"`, or `""`).                                                                                                                              | `"strict"`                  |
| flag: `--csrftoken-header`<br/>toml: `csrftoken_header`                   | string         | The name of the header for holding the CSRF token sent from the client.                                                                                                                                              | `"X-CSRF-Token"`            |
| flag: `--csrftoken-response-header`<br/>toml: `csrftoken_response_header` | string         | The name of the actual CSRF token header to return to client. If set to empty string, no CSRF token header will be set by oauth2-proxy.&nbsp;[^2]                                                     | `"X-CSRF-Token"`            |
| flag: `--skip-csrftoken-routes`<br/>toml: `skip_csrftoken_routes`         | string \| list | Bypass CSRF token validation for requests that match the method & path. Format: method=path_regex OR method!=path_regex. For all methods: path_regex OR !=path_regex. To disable CSRF token validation set to `"/*"` |                             |
| flag: `--auth-method-header`<br/>toml: `auth_method_header`               | string         | The name of the header for passing the authentication method (`"cookie"`, `"header"`) to upstream. Authentication method is part of the request scope.&nbsp;[^2]                                      | `"AuthMethod"`              |

[^2]:  CSRF token header flags are not supported when running in alpha configuration. If running with alpha configuration consult [Alpha Configuration](alpha_config.md) on how to set `injectRequestHeaders` and `injectResponseHeaders` fields accordingly.
...
| flag: `--whitelist-domain`<br/>toml: `whitelist_domains`                  | string \| list | allowed domains for redirection after authentication. Prefix domain with a `.` or a `*.` to allow subdomains (e.g. `.example.com`, `*.example.com`)&nbsp;[^3]                                                                                                                                                                                                                                                                                                                                                         |             |

[^3]: When using the `whitelist-domain` option, any domain prefixed with a `.` or a `*.` will allow any subdomain of the specified domain as a valid redirect URL. By default, only empty ports are allowed. This translates to allowing the default port of the URL's protocol (80 for HTTP, 443 for HTTPS, etc.) since browsers omit them. To allow only a specific port, add it to the whitelisted domain: `example.com:8080`. To allow any port, use `*`: `example.com:*`.
