...
Access-Token will never be refreshed, even it is already expired and a valid Refresh-Token is avilable. If set, OAuth2 Proxy will
refresh the Access-Token after this many seconds whether it is still valid or not. OAuth2 Proxy does not check for any expiry date
contained within the Access-Token. If your Access-Tokens expire and you have not set a corresponding "cookie-refresh" value, you will
likely encounter expiry issues.
