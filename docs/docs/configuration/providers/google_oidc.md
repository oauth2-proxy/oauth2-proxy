...
Unlike the default Google provider which uses Google's tokeninfo endpoint for validation, the Google OIDC provider offers OIDC-compliant ID token verification for Google authentication.

## Config Options

This provider supports all [standard OIDC options](openid_connect.md) plus the following Google-specific options:
...
| `--google-use-application-default-credentials`  | `google_use_application_default_credentials` | bool   | use application default credentials instead of service account JSON (e.g., GKE Workload Identity)  | false                                               |
| `--google-target-principal`                     | `google_target_principal`                    | string | the target principal to impersonate when using ADC                                                 | defaults to the service account configured for ADC  |

:::note
The `--google-group` option is not supported by this provider. Instead, use `--allowed-group` for group-based authorization.
:::
...
:::note
The `--oidc-issuer-url` must be set to `https://accounts.google.com`.
:::

### Setup Steps
For Google OIDC, the registration steps are:
...
Google's OIDC implementation does not include a `groups` claim in ID tokens. To support group-based authorization, this provider fetches user groups from the [Google Admin Directory API](https://developers.google.com/admin-sdk/directory).
...
8.  To restrict access to members of specific google groups, use the `--allowed-group` flag.
...
Example with groups scope enabled:
...
```
    ...
    --oidc-issuer-url=https://accounts.google.com
    --email-domain=<yourcompany.com>
    --google-admin-email=admin@yourcompany.com
    --google-service-account-json=/path/to/service-account.json
```

Note: The user's groups are fetched from the Google Admin Directory API on initial authentication and every time the token is refreshed (about once an hour).
...
```
    ...
    --allowed-group=allowed-users@yourcompany.com
```
