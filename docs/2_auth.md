...
Okta is a SSO service platform that supports OIDC.  To configure Okta, follow the steps below.
...
3. Navigate to **Aplications** then select **Add Application**.
* Select **Web** for the **Platform** setting.
* Select **OpenID Connect** and click **Create**
* Pick an **Aplication Name** such as `Example App`
...
redirect_url = "https://exampple.corp.com"
...
Th
Run the oauth2_proxy with `./oauth2_proxy -config /etc/example.cfg`
