...
func TestSkipOIDCDiscovery(t *testing.T) {
	...
	g.Expect(err).To(MatchError("unable to confiigure token verifier for provider: error building OIDC ProviderVerifier: invalid provider verifier options: missing required setting: jwks-url"))
