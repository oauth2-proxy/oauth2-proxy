...
func (p *ADFSProvider) GetLoginURL(redirectURI, state, nonce, codeChallenge, codeChallengeMethod string, extraParams url.Values) string {
	if !p.SkipNonce {
		extraParams.Add("nonce", nonce)
	}

	loginURL := makeLoginURL(p.Data(), redirectURI, url.QueryEscape(state), codeChallenge, codeChallengeMethod, extraParams)
	if p.skipScope {
		q := loginURL.Query()
		q.Del("scope")
