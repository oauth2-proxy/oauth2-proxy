...
func Test_validateCookieMinimal(t *testing.T) {
	const (
	...
	testCases := []struct {
		...
		{
			name: "No minimal cookie session",
			opts: &options.Options{
				Cookie: options.Cookie{
					Minimal: false,
				},
		...
		{
			name: "No minimal cookie session & passAuthorization",
			opts: &options.Options{
				Cookie: options.Cookie{
					Minimal: false,
				},
		...
		{
			name: "Minimal cookie session no conflicts",
			opts: &options.Options{
				Cookie: options.Cookie{
					Minimal: true,
				},
		...
		{
			name: "PassAuthorization conflict",
			opts: &options.Options{
				Cookie: options.Cookie{
					Minimal: true,
				},
		...
		{
			name: "SetAuthorization conflict",
			opts: &options.Options{
				Cookie: options.Cookie{
					Minimal: true,
				},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			errStrings := validateCookieMinimal(tc.opts)
			g := NewWithT(t)
			g.Expect(errStrings).To(ConsistOf(tc.errStrings))
		})
	}
