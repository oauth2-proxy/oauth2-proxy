...
func (t *ticket) clearCookie(rw http.ResponseWriter, req *http.Request) {
	cookieOpt := *t.options
	cookieOpt.Expire = time.Hour * -1

	http.SetCookie(rw, cookies.MakeCookieFromOptions(req, "", &cookieOpt))
}
...
func (t *ticket) makeCookie(req *http.Request, value string, expires time.Duration, now time.Time) (*http.Cookie, error) {
	...
	cookieOpt := *t.options
	cookieOpt.Expire = expires

	return cookies.MakeCookieFromOptions(req, value, &cookieOpt), nil
}
