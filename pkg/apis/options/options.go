...
func NewFlagSet() *pflag.FlagSet {
	...
	flagSet.String("redis-password", "", "Redis password. Applicable for all Redis configurations. Will override any password set in `--redis-connection-url`. Will use `REDIS_PASSWORD` environment variable if set")
	...
	flagSet.String("redis-sentinel-password", "", "Redis sentinel password. Used only for sentinel connection; any redis node passwords need to use `--redis-password`.  Will use `REDIS_SENTINEL_PASSWORD` environment variable if set")
