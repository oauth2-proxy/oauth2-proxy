...
func NewFlagSet() *pflag.FlagSet {
	...
	flagSet.String("http-store-base-url", "", "Base URL for HTTP session storage backend (eg: http://localhost:8080)")
	flagSet.String("http-store-api-key", "", "API key for authenticating with HTTP session storage backend")
