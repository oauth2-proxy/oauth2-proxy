...
type Options struct {
	...
	ExportClaims          []string `flag:"export-claim" cfg:"export_claims" env:"OAUTH2_PROXY_EXPORT_CLAIMS"`
...
func NewOptions() *Options {
	return &Options{
		...
		ExportClaims:                     []string{"sub:User", "Email"},
	}
}
...
func parseProviderInfo(o *Options, msgs []string) []string {
	...
	o.exportClaims = map[string]string{}
	// Unzip the Environment passed parameters
	if len(o.ExportClaims) == 1 {
		o.ExportClaims = strings.Fields(o.ExportClaims[0])
	}
	for _, value := range o.ExportClaims {
		parts := strings.Split(value, ":")
		if len(parts) == 1 {
			// No rename value is default
			o.exportClaims[strings.ToLower(parts[0])] = ""
		} else if len(parts) == 2 {
			o.exportClaims[strings.ToLower(parts[0])] = parts[1]
		} else {
			msgs = append(msgs, "could not parse claim mapping: "+value)
		}
	}
	return msgs
}
