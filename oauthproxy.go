...
func (p *OAuthProxy) OAuthCallback(rw http.ResponseWriter, req *http.Request) {
	...
	if errorString != "" {
		...
		if cer := p.findCallbackErrorRedirect(errorString, errorDescription); cer != nil {
			logger.Printf("Error while parsing OAuth2 callback: %q, redirecting to %q", errorString, cer.RedirectURL)
			http.Redirect(rw, req, cer.RedirectURL, http.StatusFound)
			return
		}
